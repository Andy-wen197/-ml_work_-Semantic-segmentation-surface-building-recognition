...
# mask为rle编码
train_rle = pd.read_csv('D:\\00Com_TianChi\\dataset\\train_mask.csv\\train_mask.csv',
                        sep='\t', names=['name', 'mask'])
...
for i in range(30000):
    try:
        ...
        if os.path.exists('D:\\00Com_TianChi\\dataset\\train\\build_image\\' + train_rle['name'].iloc[i]):
            cv2.imwrite('D:\\00Com_TianChi\\dataset\\train\\build_label\\' + train_rle['name'].iloc[i], train_mask)
    except:
        # 剔除异常数据
        print("存在异常文件")
        if os.path.exists('D:\\00Com_TianChi\\dataset\\train\\build_image\\' + train_rle['name'].iloc[i]):
            os.remove('D:\\00Com_TianChi\\dataset\\train\\build_image\\' + train_rle['name'].iloc[i])  # 剔除异常标签对应的图像
            print("异常rle对应的图像已剔除")
        else:
            print("异常rle对应的图像已剔除")
